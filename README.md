...
대전 중앙시장 **433개 점포·상권**을 대상으로, 사용자의 **자연어 취향**을 Gemini가 해석해 취향 vector를 만들고, 점포 feature vector와 비교해 개인화 추천을 보여주는 Next.js 웹서비스입니다.

```
AI 인터뷰 (대화 3~6문답) ─┐
                          ├─► Gemini(또는 Mock) 취향 해석 ─► UserPreferenceProfile ─► 433개 점포 매칭(cosine, 0~100점)
빠른 키워드 입력 ──────────┘                                                              │
...
- **Instagram·Meta·Google 로그인 관련 코드는 없습니다.** 외부 AI는 Gemini API만 사용하고, 서버에서만 호출합니다.
- **추천 점수·순위는 AI가 아니라 알고리즘이 계산합니다.** Gemini는 취향 해석과 설명 문장만 담당합니다.
...
| `npm test` | 단위 테스트 (추천 알고리즘, 취향 추출, 데이터 변환, Gemini·Kakao mock, 홍보 문구 검증, 보안) |
...
| 이름 | 용도 | 노출 범위 |
| --- | --- | --- |
| `GEMINI_API_KEY` | 취향 해석·추천 이유·홍보 아이디어 | **서버 전용** |
| `GEMINI_MODEL` | 비우면 `gemini-flash-latest` | 서버 |
...
| 5 | `/store/[id]` | 추천 이유, 점수 구성, 잘 맞는 취향, **원본 정보 / 추정 성향 분리 표시**, 좋아요·저장·방문·관심 없음 |
...
| — | `/admin`, `/admin/integrations` | Gemini·Supabase·Kakao·Mock 상태, 연결 테스트, 좌표 갱신, seed, 키 입력 |
...
**Gemini는 점수를 계산하지 않습니다.** 코드: `lib/recommendation/` (`engine.ts`, `feedback.ts`, `keywords.ts`, `reasons.ts`, `dimensions.ts`)
...
| --- | --- | --- |
| **Gemini** | `models/{model}:generateContent` + `responseMimeType: application/json` + `responseSchema` 구조화 출력 → Zod 검증·범위 보정 → 규칙 기반 vector와 25% 혼합. 인터뷰 다음 질문 선택, 취향 profile 생성, 키워드 의미 확장, 추천 이유, 점포 소개, 상인 홍보 아이디어. 가격·전화번호·URL·평점·퍼센트·업력(○년 전통·원조)·순위 같은 확인 불가 사실이 든 문장은 폐기 | `MockGeminiProvider` — 한국어 키워드 사전(긴 단어 우선)과 규칙 기반 슬롯 추출로 같은 구조의 결과 생성 |
| **인터뷰 안전장치** | 질문 수(최대 6)·최소 답변 수(3)·같은 항목 반복 질문 금지·같은 문장 반복 금지·연락처/URL 포함 응답 차단을 **서버 규칙이 최종 결정**합니다. Gemini가 규칙을 어기면 규칙이 고른 질문으로 대체 | 규칙 질문만으로 진행 |
...
**검증 범위(중요):** 이 코드를 만든 개발 환경은 Kakao·Google 서버에 접속할 수 없어 **실제 키로 호출하는 테스트는 하지 못했습니다.** 대신 ① 공식 문서의 요청·응답 형식으로 만든 mock 응답 단위 테스트(`tests/providers.test.ts`), ② 잘못된 키/차단된 네트워크에서 데모 모드로 대체되는지, ③ 전체 흐름(인터뷰 → 분석 → 추천 → 재분석 → 인사이트) E2E를 확인했습니다. 실제 키를 넣은 뒤 관리자 화면의 **연결 테스트**로 먼저 확인하세요.

---

## 6. Kakao / Gemini / Supabase 설정
...
2. `GEMINI_MODEL`을 비우면 `gemini-flash-latest`(최신 Flash 별칭)를 씁니다. 관리자 화면의 연결 테스트에서 사용 가능한 모델 목록을 볼 수 있습니다.

### Supabase (선택)
1. [Supabase](https://supabase.com/dashboard) 프로젝트 생성
2. **SQL Editor**에 `supabase/migrations/`의 `0001_init.sql` → `0002_store_descriptions.sql` → `0003_v2_gemini_preferences.sql`을 **순서대로** 실행
3. **Project Settings → API** 의 URL·`anon`·`service_role` 키를 환경변수로 등록 후 재배포
4. 관리자 화면 **Supabase에 점포 seed** 클릭 (또는 로컬에서 `npm run seed`)

> `0003`은 Instagram 관련 테이블을 지우고, 분석 버전 컬럼과 새 점포 컬럼을 추가하며, 이전 40개 seed(`jm-` 접두사)를 정리합니다.
...
- **Gemini 호출은 전부 서버(Route Handler)에서** 이루어집니다. 브라우저는 `/api/*`만 호출합니다.
...
supabase/migrations/      DB 스키마 SQL (0001 기본, 0002 점포 소개, 0003 v2)
