...
추천 기준(80점, 없으면 75점) 통과 점포
   └─ 좌표가 확인된 곳만 후보
        ├─ 점포 선택   : 추천 점수(연관도) 순서 그대로 — 지도 상위권과 같은 점포
        │                소분류 다양성은 점수가 같을 때만 따짐(더 높은 점포를 건너뛰지 않음)
...
- [다른 조합으로] 버튼은 확실히 더 연관된 점포는 유지하고, **마지막 자리만** 비슷한 점수(3점 이내)의 점포로 바꿉니다. 바꿀 후보가 없으면 그렇다고 알려 줍니다.
- 위치를 몰라 빠진 점포가 동선 점포보다 순위가 높으면 "추천 1위 ○○는 위치가 확인되지 않아 동선에서 빠졌어요"처럼 이름으로 알려 줍니다.
